...
class GameDomService {
  ...
  constructor() {
    ...
    this.#inputNumForm = document.getElementById(
      "inputNumForm",
    ) as HTMLFormElement;
...
class GameLogic {
  ...
  checkInput(input: number): string {
    ...
    if (this.isNumCorrect(number)) {
      this.restartGame();
      return `${this.#secretNumber} is the secret number. Count is ${
        count + 1
      }`;
    }
    if (this.isMaxCount(count + 1)) {
      this.restartGame();
      return `The max attempts has been reached. Secret Number is ${
        this.#secretNumber
      }`;
    }
    ...
    if (count === 0) {
      ...
      relativeDistanceReply =
        this.relativeDistance(number) === ComparisonResult.NOTCHANGE
          ? "Equal Distance!"
